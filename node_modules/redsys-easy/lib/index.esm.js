...
const extractAndAssertOrderFromRequestParams = (requestParams) => {
    ...
    const order = requestParams['DS_MERCHANT_ORDER'] || requestParams['Ds_Merchant_Order'];
...
const extractAndAssertOrderFromResponseParams = (responseParams) => {
    // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions, @typescript-eslint/prefer-nullish-coalescing
    const order = responseParams['Ds_Order'] || responseParams['DS_ORDER'];
    if (typeof order !== 'string' || !order) {
        throw new ParseError('Missing order number');
    }
    return order;
...
const isResponseCodeOk = (responseCode) => {
    ...
    return Number.isFinite(numResCode) && ((numResCode >= 0 && numResCode < 100) ||
 ...
 * @public
 */
const assertSuccessfulResponseCode = (responseParams) => {
...
const createRedsysAPI = (config) => {
    ...
    if (typeof config.urls !== 'object' || config.urls == null ||
...
const baseInputFormatter = (input) => {
    const { order, merchantCode, transactionType, terminal, amount, currency, merchantName, identifier, group, expiryYear, expiryMonth, pan, cvv, directPayment, merchantData, operationId, productDescription, taxReference, transactionDate, cardHolder, raw, ...unknownInput } = input;
...
const redirectInputFormatter = (input) => {
    const { merchantURL, successURL, errorURL, payMethods, lang, ...baseInput } = input;
    return {
        ...
        ...(isStringNotEmpty(lang) ? { DS_MERCHANT_CONSUMERLANGUAGE: formatLang$1(lang) } : undefined),
...
export { CARDBRANDS, COUNTRIES, CURRENCIES, GatewayError, HTTPError, LANGUAGES, PRODUCTION_URLS, ParseError, REV_CARDBRANDS, REV_COUNTRIES, REV_CURRENCIES, REV_LANGUAGES, RedsysError, ResponseError, SANDBOX_URLS, TRANSACTION_TYPES, ValidationError, assertSuccessfulResponseCode, create3DSMethodForm, create3DSv1ChallengeForm, create3DSv2ChallengeForm, createRedsysAPI, deserializeCres, deserializeThreeDSMethodData, detectSoapVersion, getResponseCodeMessage, getSISErrorCodeMessage, isResponseCodeOk, mimicSoap11NotificationResponse, mimicSoap12NotificationResponse, mimicSoapNotificationReceiver, randomTransactionId, redirectInputFormatter, restIniciaPeticionInputFormatter, restIniciaPeticionOutputFormatter, restNotificationOutputFormatter, restTrataPeticionInputFormatter, restTrataPeticionOutputFormatter, soapNotificationOutputFormatter, useOutputFormatter, usePromiseOutputFormatter, useSingleInputFormatter };
